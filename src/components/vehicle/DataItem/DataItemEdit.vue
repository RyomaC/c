<template>
  <el-form ref="form" :model="formData" label-width="100px" :class="action_type">
    <div style="text-align:left;margin-bottom:20px" v-show="action_type==='edit' || action_type==='new'">
      <el-button type="primary" @click="saveItem()">保存</el-button>
    </div>
    <component v-bind:is="formComponent" :formData="formData" v-if="formComponent"/>
    <div v-else>
    <el-form-item v-for="(field,index) in dataFields" :key="index" :label="field['title']">
      ...
      <div v-else>
        ...
        <div v-else>
          ...
          <el-input v-model="formData[field.name]" v-else />
        </div>
      </div>
    </el-form-item>
    </div>
...
export default {
  ...
  methods: {
    ...
    fetchData: function () {
      let config = require('@/components/vehicle/dataconfig/' + this.data_type + '.json')
      ...
      for (let i = 0; i < configActions.length; i++) {
        if (configActions[i].name === this.data_action) {
          ...
          if (configActions[i]['component']) {
            this.formComponent = () => import('@/components/vehicle/DataItem/' + configActions[i]['component'])
          }
          break
        }
      }
      ...
      this.$axios
        ...
        .then(response => {
          ...
          if (response.data.errno > 0) {
            ...
            this.$EventBus.$emit('dataItemUpdated', this.data_type)
            if (!submitId) {
              this.$emit('hideItem')
            }
          }
...
.el-form-item, .el-form-item__label, .el-form-item__content, .form-item-value{
  height: 20px;
}
</style>
