<template>
  <el-form label-width="80px" style="height:100%" :class="data_type">
    ...
    <component style="padding:5px 20px 10px 10px; height:calc(100% - 120px);overflow-y:auto; overflow-x:hidden;" v-bind:is="itemActionComponent" :data_id="data_id" :data_type="data_type" :data_action="currentAction" :action_type="actionType" :data="data" @itemUpdated="itemUpdated" @hideItem="hideItem"></component>
  </el-form>
</template>

<script>
import DataItemEdit from '@/components/DataItem/DataItemEdit'
  ...
  methods: {
    ...
    loadComponent: function () {
      ...
      if (this.actionType === 'component') {
        this.itemActionComponent = () => import('@/components/DataItem/' + this.data_type + '_' + this.currentAction)
      } else {
